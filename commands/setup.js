...
module.exports.run = async (client, message, args) => {
    ...
    if (!args[0]) {
        ...
        client.functions.setup_mute_channel(client, message)
    } else if (args[0].toLowerCase() === "autorole" || args[0].toLowerCase() === "auto-role") {
        client.functions.setup_autorole(client, message)
    } else if (args[0].toLowerCase() === 'prefix') {
        client.functions.setup_prefix(client, message)
    } else if (args[0].toLowerCase() === 'modlogs') {
        client.functions.setup_modLogs(client, message)
    }
